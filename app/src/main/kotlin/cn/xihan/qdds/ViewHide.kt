...
fun PackageParam.hideBookshelfDailyReading(versionCode: Int) {
    val gridAdapterClass = when (versionCode) {
        ...
        in 884..938 -> "com.qidian.QDReader.ui.adapter.g0"
        else -> null
    }
    val listAdapterClass = when (versionCode) {
        ...
        in 884..938 -> "com.qidian.QDReader.ui.adapter.i0"
        else -> null
    }
    ...
    when (versionCode) {
        in 868..938 -> {
            ...
            val needHookClass = when (versionCode) {
                ...
                in 884..938 -> "com.qidian.QDReader.ui.viewholder.bookshelf.o"
                else -> null
            }
            needHookClass?.hook {
                injectMember {
...
fun PackageParam.hideBottomRedDot(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        in 884..938 -> "com.qidian.QDReader.ui.widget.maintab.b"
        else -> null
    }
...
fun PackageParam.removeQSNYDialog(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        in 932..938 -> "com.qidian.QDReader.bll.helper.m0"
        else -> null
    }
    needHookClass?.hook {
        injectMember {
