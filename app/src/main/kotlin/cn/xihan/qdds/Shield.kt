...
fun PackageParam.shieldDailyReading(
    ...
    val needHookClass = when (versionCode) {
        ...
        980 -> "com.qidian.QDReader.component.api.j1"
        else -> null
    }
    ...
    when (versionCode) {
        ...
        in 827..980 -> {
            val needHookClass = when (versionCode) {
                ...
                980 -> "com.qidian.QDReader.ui.adapter.z6"
                else -> null
            }
            val needHookMethod = when (versionCode) {
                in 827..878 -> "r"
...
fun PackageParam.shieldHotAndRecommend(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        980 -> "com.qidian.QDReader.ui.adapter.q"
        else -> null
    }
    needHookClass?.hook {
        injectMember {
    ...
    when (versionCode) {
        ...
        in 804..980 -> {
            ...
            val needHookClass = when (versionCode) {
                ...
                980 -> "com.qidian.QDReader.ui.adapter.tb"
                else -> null
            }
            val needHookMethod = when (versionCode) {
                in 804..878 -> "q"
...
fun PackageParam.shieldComicOther(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        980 -> "com.qidian.QDReader.ui.adapter.a2"
        else -> null
    }
    val needHookMethod = when (versionCode) {
        in 812..878 -> "q"
