...
fun PackageParam.shieldDailyReading(
    ...
    val needHookClass = when (versionCode) {
        ...
        in 896..932 -> "com.qidian.QDReader.component.api.i1"
        else -> null
    }
    ...
    when (versionCode) {
        ...
        in 827..932 -> {
            val needHookClass = when (versionCode) {
                ...
                932 -> "com.qidian.QDReader.ui.adapter.x6"
                else -> null
            }
            val needHookMethod = when (versionCode) {
                in 827..878 -> "r"
...
fun PackageParam.shieldHotAndRecommend(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        in 884..932 -> "com.qidian.QDReader.ui.adapter.r"
        else -> null
    }
    needHookClass?.hook {
        injectMember {
    ...
    when (versionCode) {
        ...
        in 804..932 -> {
            ...
            val needHookClass = when (versionCode) {
                ...
                932 -> "com.qidian.QDReader.ui.adapter.rb"
                else -> null
            }
            val needHookMethod = when (versionCode) {
                in 804..878 -> "q"
...
fun PackageParam.shieldComicOther(versionCode: Int) {
    val needHookClass = when (versionCode) {
        ...
        in 884..932 -> "com.qidian.QDReader.ui.adapter.b2"
        else -> null
    }
    val needHookMethod = when (versionCode) {
        in 812..878 -> "q"
